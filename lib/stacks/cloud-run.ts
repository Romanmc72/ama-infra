...
export class CloudRunStack extends BaseGCPStack {
  ...
  constructor(
    ...
    // and register a CNAME record for the new subdomain. The settings for
    // name cheap for prod were as follows:
    // Add a new DNS record to the settings.
    // Use record type "CNAME"
    // For the "host" use the subdomain (for prod that is "ama")
    // for value use "ghs.googlehosted.com"
    // Those settings may change, but that is what was required as of
    // writing this when navigating to the domain mapping page on cloud run.
    new CloudRunDomainMapping(this, 'domain-map', {
      name: `${env.isProd ? '' : env.name}ama.r0m4n.com`,
      location: env.region,
      metadata: {
